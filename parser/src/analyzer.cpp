...
auto Anaylzer::isStatement(const std::vector<LXR_NS::Token>& tokens) const noexcept -> bool{

    return false;
}
 
auto Anaylzer::isExpression(const std::vector<LXR_NS::Token>& tokens) const noexcept -> bool{
    if (tokens.size() == 0)
        return false;
    if (tokens.size() == 1)
    ...
    if (tokens.size() == 2)
        return false;
    if (tokens.size() == 3)
         return (tokens[1].strValue == "-" || tokens[1].strValue == "+") && isFactor(tokens[0].strValue) && isFactor(tokens[2].strValue); 

    return isFactor(tokens[0].strValue) && isExpression({tokens.begin()+2, tokens.end()});
}
